...
def init_db():
    ...
    if db_mode == "sqlite":
        ...
        CREATE TABLE IF NOT EXISTS users (
            ...
            username TEXT UNIQUE NOT NULL,
            email TEXT UNIQUE NOT NULL,   -- strict in prod
            password TEXT NOT NULL,
...
def process_chat_job(job_id, user_id, tier, user_msg):
    """
    Background worker: call route_ai_call, wait artificial delay,
    save to DB if user_id present, then mark job done.
    """
    try:
        reply = route_ai_call(tier, user_msg)
    except Exception as e:
        log_suspicious("LLMErrorBackground", str(e))
        reply = f"(fallback) I heard: {user_msg}"

    # Decide delay (5-10s) or 1s if the user requested "fast"
...
def _openrouter_chat(prompt: str, model: str = "openrouter/auto") -> Optional[str]:
    ...
    try:
        ...
        headers = {
            ...
            "HTTP-Referer": "https://evosgpt.one",  # required by OpenRouter
            "X-Title": "EVOSGPT",
            "Content-Type": "application/json"
        }
        data = {"model": model, "messages": [{"role": "user", "content": prompt}]}
...
def gpt3_5_turbo(prompt: str) -> str:
    return _openai_chat(prompt, "gpt-3.5-turbo") or _openrouter_chat(prompt, "openai/gpt-3.5-turbo") or f"[3.5-Echo] {prompt}"

def gpt4o_mini(prompt: str) -> str:
    return _openai_chat(prompt, "gpt-4o-mini") or _openrouter_chat(prompt, "openai/gpt-4o-mini") or local_llm(prompt) or f"[Mini-Echo] {prompt}"

def gpt4o(prompt: str) -> str:
    return _openai_chat(prompt, "gpt-4o") or _openrouter_chat(prompt, "openai/gpt-4") or local_llm(prompt) or f"[4o-Echo] {prompt}"

# --- Router (main entry) ---
def route_ai_call(tier: str, prompt: str) -> str:
    """
    Decide which AI model to use based on tier + fallbacks.
    Adds logging so you know what fired.
    """
    ...
    # Default
    return f"[UnknownTier:{tier}] {prompt}"
...
def index():
    ...
    if show_memory and session.get("user_id"):
        try:
            ...
            # ✅ User chat history (last 100, oldest → newest)
            c.execute("""
...
def chat():
    ...
    # --- Guest mode (limit to 5 messages) ---
    if "user_id" not in session:
        guest_count = session.get("guest_count", 0)
        # warn on 4th chat can be handled client-side if you want to display special msg
        if guest_count >= 5:
            # You can return a redirect URL here or an error code for client to redirect
            return jsonify({"error": "Guest limit reached", "redirect": "/login"}), 403
        session["guest_count"] = guest_count + 1

    # --- Founder unlock/logout logic (runs synchronously so it can change session immediately) ---
    ...
    if "user_id" in session:
        ...
        if seq == 0 and ui == "evosgpt where you created":
            ...
            immediate_reply = "are you coming to Ghana?"
            session["founder_seq"] = 2
        elif seq == 2 and ui == "nameless":
            immediate_reply = "[SYSTEM] Founder tier unlocked. Welcome, hidden user."
            ...
            immediate_reply = "[SYSTEM] Founder mode deactivated. Returning to Basic tier."
            session["tier"] = "Basic"
            try:
                conn = sqlite3.connect("database/memory.db")
                c = conn.cursor()
            ...
            log_action(session["user_id"], "Founder Logout", "Founder manually downgraded to Basic")
        else:
    ...
    # If founder command produced immediate_reply, save it and return done
    if immediate_reply is not None:
        # If logged in, save chat immediately
        if "user_id" in session:
            try:
                conn = sqlite3.connect("database/memory.db")
                c = conn.cursor()
                c.execute(
                    "INSERT INTO memory (user_id, user_input, bot_response, system_msg) VALUES (?, ?, ?, 0)",
                    (session["user_id"], user_msg, immediate_reply)
                )
                conn.commit()
                c.execute("INSERT INTO activity_log (user_id, action, details) VALUES (?, ?, ?)",
                          (session["user_id"], "chat", f"User: {user_msg} | Bot: {immediate_reply}"))
                conn.commit()
                enforce_memory_limit(session["user_id"], session.get("tier", "Basic"))
                conn.close()
            except Exception as e:
                log_suspicious("ChatInsertFailImmediate", str(e))

        with jobs_lock:
            jobs[job_id] = {"status": "done", "reply": immediate_reply}
        return jsonify({"job_id": job_id, "status": "done"})

    # otherwise start background worker
    with jobs_lock:
        jobs[job_id] = {"status": "processing"}

    user_id = session.get("user_id")
    worker = threading.Thread(target=process_chat_job, args=(job_id, user_id, tier, user_msg), daemon=True)
    worker.start()
...
def register():
    if request.method == "POST":
        ...
        # ✅ (Optional) restrict to common domains
        ...
        try:
            ...
            # 🔹 First 100 users → Pro, rest → Basic
            ...
            c.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (username, email, hashed_pw, tier))
            conn.commit()
            conn.close()

            # 🔹 Mirror to Supabase (backup sync)
            try:
